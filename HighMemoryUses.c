    ...
    if (Process32First(processSnapshot, &processEntry)) {
        do {
            ...
            if (pmc.WorkingSetSize > highMemThreshold) {
                ...
                HANDLE processTreeSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, processEntry.th32ProcessID);
                if (processTreeSnapshot == INVALID_HANDLE_VALUE) {
                    CloseHandle(processHandle);
                    continue;
                }

                PROCESSENTRY32 processTreeEntry;
                processTreeEntry.dwSize = sizeof(processTreeEntry);
                ...
                CloseHandle(processTreeSnapshot);
            }
